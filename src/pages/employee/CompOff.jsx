...
import { Calendar, Info, Clock, AlertCircle, CheckCircle, XCircle } from 'lucide-react'
...
export default function EmployeeCompOff() {
  ...
  const validate = () => {
    ...
    if (!form.worked_date) {
      ...
      // Check if selected day is Saturday (getDay() === 6 in UTC or local depending on parsing)
      // Since input date is in YYYY-MM-DD local format, let's parse it correctly
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Form */}
        <div className="lg:col-span-1 bg-slate-900/40 backdrop-blur-md rounded-2xl border border-slate-800 p-5 space-y-4 h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Textarea
              ...
              error={errors.reason}
              placeholder="Specify the work performed on Saturday…"
              rows={3}
              labelClassName="text-slate-300"
            />
        ...
        {/* History List */}
        <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-md rounded-2xl border border-slate-800 p-5 space-y-4">
          <h2 className="font-semibold text-lg text-slate-200">Request History</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800 text-slate-400">
                  <th className="text-left py-3 text-xs font-semibold uppercase">Worked Date</th>
                  <th className="text-left py-3 text-xs font-semibold uppercase">Credited Days</th>
                  <th className="text-left py-3 text-xs font-semibold uppercase">Reason</th>
                  <th className="text-left py-3 text-xs font-semibold uppercase font-sans">Applied On</th>
                  <th className="text-right py-3 text-xs font-semibold uppercase">Status</th>
                </tr>
              </thead>
                ...
                {requests.length === 0 ? (
                  ...
                  requests.map(r => (
                    <tr key={r.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="py-3 font-medium">{formatDate(r.worked_date)}</td>
                      <td className="py-3">{r.credited_days}</td>
                      <td className="py-3 text-slate-400 max-w-[200px] truncate" title={r.reason}>{r.reason}</td>
                      <td className="py-3 text-slate-400 text-xs">{formatDate(r.created_at?.split('T')[0])}</td>
          ...
          <div className="block md:hidden space-y-3">
            {requests.length === 0 ? (
              ...
              requests.map(r => (
                <div key={r.id} className="border border-slate-800 rounded-xl p-4 bg-slate-900/20 space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="font-semibold text-slate-200">{formatDate(r.worked_date)}</p>
                    <StatusBadge status={r.status} />
                  </div>
                  <p className="text-xs text-slate-400">Credited Days: {r.credited_days} · Applied: {formatDate(r.created_at?.split('T')[0])}</p>
                  <p className="text-sm text-slate-300 leading-snug">{r.reason}</p>
                </div>
              ))
            )}
          </div>
